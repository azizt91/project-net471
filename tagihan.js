...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function markAsPaid(invoice) {
        ...
        try {
            ...
            // Generate receipt before fetching new data
            const otherUnpaidBills = unpaidData.filter(b => b.IDPL === invoice.IDPL && b.rowNumber !== invoice.rowNumber);
            if (confirm('Tagihan berhasil dilunaskan! Apakah Anda ingin mencetak struk?')) {
                printReceipt(invoice, otherUnpaidBills);
            }
